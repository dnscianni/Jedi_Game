	...
	public static void main(String[] args) {
		...
		do {
			...
			if (resp == 1) { // when resp equals 1 means user wants to move
				...
				gameMap.setMaze(user.getLocation()[1] * 4 + 3,
						user.getLocation()[0] * 4 + 3, " ");
				// erases old position of the display

				if (resp > 0) {
					user.move(resp, gameMap); // moves user
				}
				gameMap.setMaze(user.getLocation()[1] * 4 + 3,
						user.getLocation()[0] * 4 + 3, "P");// sets P to current
															// location on the
															// display grid
				...
				for (int i = 0; i < masters.length; i++) {
					if ((masters[i].getLocation()[0] == user.getLocation()[0] && masters[i]
							...
							|| (masters[i].getLocation()[0] == user
									...
									.getLocation()[1] == user.getLocation()[1] + 1)) {
						gameMap.setMaze(masters[i].getLocation()[1] * 4 + 3,
								masters[i].getLocation()[0] * 4 + 3, "J");
					}
				}
				gameMap.setMaze(user.getLocation()[1] * 4 + 3,
						user.getLocation()[0] * 4 + 3, "P");
				...
				if (gameMap.getArea(user.getLocation()[0], // checks if the user
					...
					try {
						...
						gameMap.setMaze(h.getLocation()[0], h.getLocation()[1],
								"H");
						gameMap.printMaze();
			...
			if (resp == 2) { // if resp equals 2 means user wants to look in a
				...
				gameMap.printMaze(); // prints maze with current location
				System.out.println("There are " + user.look(resp, gameMap)
				// if a jedi is present, it will say how many
			...
			if (resp == 3) { // 3 means user wants to shoot
				...
				while (resp > 4 || resp <= 0) { // keeps running into input is
												// within range
					System.out
							.println("What's wrong with you? Put in a correct number! ");
					resp = getInput(resp);
				}
	...
	public static void mastersMove(Map gameMap, Jedi[] masters) {
		...
		for (int i = 0; i < masters.length; i++) {
			...
			while ((z == 1 && masters[i].getLocation()[1] - 1 >= 0 && gameMap
					...
					// this and the next four conditions check to see whether it
					// is moving into a room
					...
					// this and the next four conditions check to see if they're
					...
					// this and the next few conditions check to ensure that
					// they do not move into an Area with a Person
					|| (z == 2 && gameMap.getArea(
							masters[i].getLocation()[0] + 1,
