...
public class Game {
	...
	/**
	 * The main method of the game class will create and run the game aspects of
	 * our assignment. It will implement all the objects that are needed in the
	 * game and set up the user inputs so that the player can play the game.
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		/* Instantiates all the necessary fields in our game */
		...
		/*
		 * This prints out the story of our game, and the instructions on how to
		 * play the game. It also sets up the main menu, which gives the user
		 * the option of either loading a saved game, starting a new game,
		 * starting a new game in debug mode or quitting the game.
		 */
		System.out
				.println("====================================================================================================");
		System.out
		...
		/*
		 * this will run until the player gives an acceptable response to the
		 * main menu. If 0 is pressed, then it will start a new game will all
		 * new objects. If 1 is pressed it will load a saved game and set all
		 * the objects to the ones from the save file. If 2 is pressed, then it
		 * will start a new game, just like if they pressed 0, but it will set
		 * isDebug to true, so that we will be in debug mode. Lastly, if they
		 * choose -1, then the game will terminate.
		 */
		do {
			resp = getInput(resp);
		...
		/*
		 * random numbers are used to set locations of our Holocron and Jedis
		 */
		Random r = new Random();
		int x = r.nextInt(9);
		int y = r.nextInt(9);
		/*
		 * this will give Jedi's new locations if the game is not loaded froma a
		 * save file.
		 */
		if (resp != 1) {
			masters = new Jedi[6];
			...
			for (int i = 0; i < masters.length; i++) {
				...
				/*
				 * restrict jedis from spawning too close to the player
				 */
				...
				/*
				 * creates masters[i] of type Jedi. Also sets location of
				 * masters[i]
				 */
				masters[i] = new Jedi(x, y);
		...
		/* each number between 0-8 is associated with a room */
		if (resp != 1) {
			switch (z) {
			case 0:
				h = new Holocron(1, 1);
		...
		/*
		 * sets location of users on the display grid
		 */
