...
public class Player extends Person {
	/**
	 * The default serial ID of the Player class.
	 */
	 ...
	 * if b is true then it will reset the maze location to an empty space, but
	 * if not it will set it to a #.
	 * 
	 * @param m
	 *            The map the game uses, so that the players location on the map
	 *            can change
	 * @param b
	 *            determines if the game is in debug mode or not.
	 ...
	 * 
	 * @param m
	 *            the map the game uses so that the player's location in the map
	 *            can change.
	 * @param d
	 ...
	 * exception. If the bullet tries to go through a wall or out of bounds it
	 * will stop the bullet from moving any further.
	 * 
	 * @param d
	 *            The direction in which the player shoots his or her blaster.
	 * @param m
	 *            The map the game uses so that it can check the locations of
	 *            the persons involved.
	 * @param b
	 *            Used to check to see if the game is in debug mode or not.
	 * @return the amount of jedi that died with the shot.
	 */
			...
			case 1:
				for (int i = this.location[1] - 1; i >= 0; i--) {
					...
					if (m.getArea(this.location[0], i).getPerson() != null) {
						try {
							m.getArea(this.location[0], i).getPerson()
									.die(m, b);
							temp++;
			...
			case 2:
				for (int i = this.location[0] + 1; i <= 8; i++) {
					...
					if (m.getArea(i, this.location[1]).getPerson() != null) {
						try {
							m.getArea(i, this.location[1]).getPerson()
									.die(m, b);
							temp++;
						} catch (GameOverException e) {
							e.printStackTrace();
	 ...
	 * given. It will print any jedi or items that are in the spaces being
	 * looked at on the map. It will also check to make sure you are not trying
	 * to look through any walls or into a room.
	 * 
	 * @param d
	 *            The direction in which the player looks
	 * @param m
	 *            The map the game uses so that the areas in the map can be
	 *            called.
	 * @param j
	 *            The jedi array, so that the game can check to see if any jedi
	 *            are within the spaces you are looking.
	 * @param a
	 *            The AddAmmoPoweUp, so that the game can check to see if the
	 *            power up is within the spaces you are looking.
	 * @param in
	 *            The InvincibilityPoweUp, so that the game can check to see if
	 *            the power up is within the spaces you are looking.
	 * @param r
	 *            The RadarPoweUp, so that the game can check to see if the
	 *            power up is within the spaces you are looking.
	 */
	...
	/**
	 * This will return the gun that the Player uses.
	 * 
	 * @return The Gun blaster
	 */
	public Gun getGun() {
		return blaster;
	}
