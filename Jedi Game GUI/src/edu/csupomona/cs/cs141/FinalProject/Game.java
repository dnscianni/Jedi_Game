...
public class Game {
	...
	public static void main(String[] args) {
		...
		gameMap = new Map(isDebug); // creates gameMap of type Map
		if (resp != 1) {
			user = new Player(); // creates object of type Player to
		} // represent the user
		int temp = 0;
		...
		System.out.println("Would you like to play in normal text mode, or"
				+ " play via GUI? (0: normal, 1: GUI)");
		resp = -1;
		...
		if (resp == 0)
			...
			normalTextMode = false;

		if (normalTextMode) {
			gameMap.setMaze(user.getLocation()[1] * 2 + 1,
					user.getLocation()[0] * 4 + 2, "P");// sets P to current
														// location on display
														// grid
		...
		if (!normalTextMode) {
			GUIDisplay = new GUIMap(user, masters, invinc, radar, addAmmo, h,
					isDebug, gameMap, saveState);
		}

		while (normalTextMode) {
			...
			if (resp == 1) { // when resp equals 1, then user wants to move
				...
				if ((temp - 1) > 0) {
					System.out.println("you have " + (temp - 1)
