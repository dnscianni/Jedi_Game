...
public class GUIMap extends JFrame {
	...
	public GUIMap(Player p, Jedi[] jed, InvincibilityPowerUp inv,
			RadarPowerUp r, AddAmmoPowerUp a, Holocron h, boolean noFog, Map m,
			Save s) {
		setTitle("(Insert game's name here)");
		...
		titleImage = new ImageIcon("Star_Wars.png");
		title = new JLabel(titleImage);
		layout.putConstraint(SpringLayout.NORTH, title, 3, SpringLayout.NORTH,
				mainPanel);
		layout.putConstraint(SpringLayout.WEST, title, 400, SpringLayout.WEST,
				mainPanel);
		mainPanel.add(title);
		...
		layout.putConstraint(SpringLayout.NORTH, mapGrid[0][0], 3,
		...
		layout.putConstraint(SpringLayout.NORTH, mainText, 5,
	...
	public void endTurn() {
		...
		Game.mastersMove(gameMap, masters);
	}
