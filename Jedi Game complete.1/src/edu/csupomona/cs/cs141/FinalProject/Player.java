...
public class Player extends Person {
	...
	public void die(Map m) throws GameOverException {
		if (lives == 1) {
			...
			m.setMaze(this.location[0] * 2 + 1, this.location[1] * 4 + 2, "#");
			this.location[0] = 0;
	...
	public int look(int d, Map m) {
		...
		switch (d) {
		case 1:
			if (!m.getArea(this.location[0], this.location[1] - 1).getIsRoom()) {
			...
			break;
		case 2:
			if (!m.getArea(this.location[0] + 1, this.location[1]).getIsRoom()) {
