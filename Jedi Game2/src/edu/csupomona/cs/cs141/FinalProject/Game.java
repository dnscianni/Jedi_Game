...
public class Game {
	...
	public static void main(String[] args) {
		...
		if (resp != 1) {
			...
			for (int i = 0; i < masters.length; i++) {
				...
				masters[i] = new Jedi(x, y, false); // creates masters[i] of type Jedi
		...
		if (GUI)
			new GUIClass(user, masters, invinc, radar, addAmmo, h,
				isDebug, gameMap, saveState);
		...
		while (!GUI) {
			...
			if (resp == 1) { // when resp equals 1 means user wants to move
				...
				mastersMove(gameMap, masters, user);
				...
				if (hasDied) {
					mastersStopChasing(masters); // part of the AI
	...
	public static void mastersMove(Map gameMap, Jedi[] masters, Player user) {
		...
		for (int i = 0; i < masters.length; i++) {
			...
			if (z < 5) {
				...
				masterLookAround(gameMap, masters, masters[i]);  // part of the AI
	...
	public static boolean mastersStab(Jedi[] masters, boolean isDebug,
		...
		for (int i = 0; i < masters.length; i++) {
			...
			hasDied = masters[i].stab(user, gameMap, isDebug, GUI); // jedi
			// stabs
			if (hasDied) {
				break;
			}
	 ...
	 // part of the AI
	public static void masterLookAround(Map gameMap, Jedi[]
	 masters, Jedi whichJedi) {
		boolean isJedi = true;
		...
		if ((whichJedi.getLocation()[0] > 0
			...
			for (int j = 0; j < masters.length; j++) {
				if (gameMap.getArea(whichJedi.getLocation()[0] - 1,
						whichJedi.getLocation()[1]).getPerson()
						== masters[j]) {
					isJedi = true;
					break;
				}
						...
						whichJedi.getLocation()[1] - 1).getIsRoom()) {
							...
							for (int j = 0; j < masters.length; j++) {
								if (gameMap.getArea(whichJedi.getLocation()[0],
										whichJedi.getLocation()[1] - 1).getPerson()
										== masters[j]) {
									isJedi = true;
									break;
								}
