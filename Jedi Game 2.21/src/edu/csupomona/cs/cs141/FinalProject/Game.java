...
public class Game {
	...
	public static void main(String[] args) {
		...
		maze[h.getLocation()[1] * 4 + 1][h.getLocation()[0] * 4 + 1] = "H"; // sets
																			...
																			// display
																			// grid
		maze[user.getLocation()[1] * 4 + 3][user.getLocation()[0] * 4 + 3] = "P"; // sets
																					// location
																					// of
																					// users
																					// on
		...
		for (int i = 0; i < masters.length; i++) {
			maze[masters[i].getLocation()[1] * 4 + 3][masters[i].getLocation()[0] * 4 + 3] = "J"; // sets
			// location
			// of
			// masters[i]
			// on
		...
		do {
			...
			if (resp == 1) { // when resp equals 1 means user wants to move
				if (gameMap.getArea(user.getLocation()[0],
					...
					System.out.println("up(1),right(2),down(3),left(4) or stay in place(0)?: ");
					resp = scn.nextInt();
				}
				...
				maze[user.getLocation()[1] * 4 + 3][user.getLocation()[0] * 4 + 3] = " "; // erases
																							// old
																							// position
																							// of
																							// the
				...
				for (int i = 0; i < masters.length; i++) {
					...
					if (masters[i].getLocation()[1] >= 0) { // h
						maze[masters[i].getLocation()[1] * 4 + 3][masters[i]
								.getLocation()[0] * 4 + 3] = " ";
					}
				}
				if (resp > 0) {
			...
			if (resp == 3) { // 3 means user wants to shoot
				...
				System.out.println("You heard " + user.shoot(resp, gameMap) + " scream(s)"); // users shoots in a direction
				maze[h.getLocation()[1] * 4 + 1][h.getLocation()[0] * 4 + 1] = "H"; // sets
																					// location
																					// of
																					// the
