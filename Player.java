...
public class Player extends Person {
	...
	public int look(int d, Map m) {
		...
		case 1:
			if (location[1] - 1 >= 0) {
				if (!m.getArea(location[0], location[1] - 1).getIsRoom()) {
					...
					if (!m.getArea(this.location[0], this.location[1] - 2)
						...
						if (this.location[1] - 2 >= 0) {
							if (m.getArea(this.location[0],
									this.location[1] - 2).getPerson() != null) {
								temp++;
							}
						}
					}
					...
					if (location[0] + 2 <= 8) {
						if (!m.getArea(this.location[0] + 2, this.location[1])
							...
							if (this.location[0] + 2 <= 8) {
								if (m.getArea(this.location[0] + 2,
										this.location[1]).getPerson() != null) {
									temp++;
								}
							}
						}
